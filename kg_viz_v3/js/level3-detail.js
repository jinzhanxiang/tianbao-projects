...
const Level3Detail = (function() {
      ...
      fetchWikiEntityData(name, function(data) {
        ...
        if (wikiLink) {
          wikiLink.onclick = function() {
            // KG 独立站点的 Wiki 链接（tianbao-kg/kg_viz_v3/wiki/wiki-viewer.html）
  ...
  function buildFullEntityDetail(d) {
    ...
    // Wiki 页面链接（主路径也需要）
    html += `<div class="detail-card" style="margin-top:8px;text-align:center;padding:10px">
      <p style="color:#1d9bf0;cursor:pointer;margin:0;font-size:13px" id="wiki-link">📄 查看 Wiki 页面</p>
    </div>`;
